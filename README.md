...
Copy `env-template.toml` and create your own `env.toml` file and update the following variables:
- `RPC`: RPC endpoint of the preferred network for this test (recommended network is Ethereum Sepolia).
- `PRIVATE_KEY`: your private key without the 0x prefx.
- `FROM_ADDRESS`: the address the private key owns.
...
This test repo imports Transaction Manager v2 from Chainlink-EVM (`github.com/smartcontractkit/chainlink-evm/pkg/txm`).
...
- Keystore
- InMemory Layer
- (Optional) Stuck Transaction Detector
